...
describe("checkpointService.commit", () => {
  it("commits a checkpoint for a valid, fresh observation", async () => {
    observationRepositoryMock.getById.mockResolvedValueOnce(observation());

    const [outcome] = await checkpointService.commit([{ symbol: "INFY.NS", observationId: "obs-1" }]);
  ...
  it("refuses to checkpoint when the observation can't be found (no silent no-op)", async () => {
    ...
    const [outcome] = await checkpointService.commit([{ symbol: "INFY.NS", observationId: "missing" }]);
  ...
  it("processes multiple items independently, one bad item doesn't block the rest", async () => {
    ...
    const outcomes = await checkpointService.commit([
