...
**Live app:** https://smart-market-watchlist-one.vercel.app/
...
actually changed since, backed by a deterministic scoring engine rather
than an LLM.
...
This is **market-data/testing infrastructure**, not just cosmetic. Forcing
a scenario never assigns a classification directly: it
...
user's _classification_ of that shared move can still differ.
...
from the switcher. The only thing that's device-local is _which_ demo user
you're currently viewing as (a plain `localStorage` value). Following that
across devices would require some form of login, which is exactly what
...
watching it (see "Multi-user" above). A dashboard load fans out one
provider call per watchlist symbol; with the synthetic provider that's
just a local, deterministic computation, but a real provider behind
`IMarketDataProvider` would turn that fan-out into a real external-request
concern needing its own rate limiting. The UI also has no pagination; fine
for a handful of symbols, but a version built for dozens-plus per user
would want to batch that fan-out into a single provider call and
virtualize the list.

## Running locally

```bash
cp .env.example .env      # DATABASE_URL + DIRECT_URL, both pointing at the db container below
...
2. Add a Postgres database (Vercel's Storage tab, or any hosted Postgres,
   such as Neon or Supabase) and set two environment variables on the
   project:
   - `DATABASE_URL`: a _pooled_ connection string, if your provider offers
     ...
     the standard fix.
   - `DIRECT_URL`: the same database's _unpooled/direct_ connection
     ...
     pooled (PgBouncer transaction-mode) connection can't reliably hold, so
